...
def run_full_accuracy_benchmark() -> Dict[str, Any]:
    ...
    try:
        ...
        # Field-level confusion / result table for Dataset F (Step 3 verification)
        f_item = next((item for item in gt_data if item.get("id") == "F_unknown_trade_delimited"), None)
        f_confusion_rows = []
        if f_item:
            f_events = engine.ingest_text(f_item["raw"], persist=False)
            f_ev = f_events[0] if f_events else None
            f_unmapped = f_ev.unmapped if f_ev else {}
            f_dict = f_ev.model_dump() if f_ev else {}
            for k, exp_val in f_item.get("expected_fields", {}).items():
                act_val = f_dict.get(k) or f_unmapped.get(k)
                match = _compare_field_value(act_val, exp_val, k)
                f_confusion_rows.append({
                    "expected_field": k,
                    "expected_value": exp_val,
                    "actual_field": k if act_val is not None else "None",
        ...
        summary = {
            ...
            "dataset_f_confusion_matrix": f_confusion_rows,
        }
...
"""
        ...
        print("\n" + "=" * 80)
        print("DATASET F (TRADE LOG) FIELD-LEVEL CONFUSION / RESULT TABLE")
        print("=" * 80)
        print(f"{'Expected Field':<16} | {'Expected Value':<22} | {'Actual Field':<16} | {'Actual Value':<22} | {'Match':<5}")
        print("-" * 80)
        for row in f_confusion_rows:
            print(f"{row['expected_field']:<16} | {row['expected_value']:<22} | {row['actual_field']:<16} | {row['actual_value']:<22} | {'MATCH' if row['match'] else 'MISMATCH'}")
        print("=" * 80)
